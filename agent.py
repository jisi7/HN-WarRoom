...
FLUX Dev for realistic photography. Simplified, real-world photo prompts.
"""
...
# Claude fills in the specific subject from article context.
PHOTO_STYLE = (
    ...
    "Clean minimal composition with room for text overlay. "
...
PRODUCT_PHOTO_SUBJECTS = {
    "creatine": [
        ...
        "close-up of human muscle tissue cross-section on a lab slide under soft studio lighting",
    ...
    "focase": [
        ...
        "close-up of a person's hands typing on a keyboard with coffee beside them, warm office light",
        ...
        "close-up of human brain MRI scan on a lightbox in a dark clinical room",
...
{
  "title": "...",
  "photo_subject": "Describe ONE concrete real-world subject to photograph for this article — a physical object, material, or scene that relates to the topic. Examples: 'close-up of white creatine powder in a ceramic scoop on white surface', 'sunlight streaming through a window onto a desk', 'athlete gripping a pull-up bar'. Keep it simple and photographable.",
...
def build_photo_prompt(article, product):
    """
    Combine article's photo_subject (simple real-world object/scene) with
    FLUX Dev photographic style direction. Falls back to product-specific
    curated subjects if article didn't return a usable subject.
    """
    subject = (article.get("photo_subject") or "").strip()

    # Reject abstract or molecular descriptions — fall back to curated list
    bad_signals = ["molecular", "cross-section", "cellular", "atomic", "microscop",
                   "diagram", "illustration", "abstract", "symbolic"]
...
def generate_hero_image(article, product):
    ...
    if REPLICATE_API_KEY:
        try:
            ...
            # FLUX Dev — more realistic, better prompt adherence than Schnell
            r = requests.post(
